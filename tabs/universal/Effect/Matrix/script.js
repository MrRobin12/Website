...
// requestAnimationFrame polyfill by Erik MÃfÆ'Ã?â?TÃfâ?sÃ,Â¶ller
...
var M = {
  ...
  settings: {
	...
	init: function () {
		...
		M.c.width = M.WIDTH;
		M.c.height = M.HEIGHT + 500;

		M.ctx.shadowBlur = 0;
		M.ctx.fillStyle = '#000';
		M.ctx.fillRect(0, 0, M.WIDTH, M.HEIGHT + 500);
		M.ctx.font = M.font;
		...
		window.onresize = function () {
			...
			M.animation = null;
			M.ctx.clearRect(0, 0, M.WIDTH, M.HEIGHT + 500);
			M.codesCounter = 0;

			M.ctx2.clearRect(0, 0, M.WIDTH, M.HEIGHT + 500);

			M.WIDTH = window.innerWidth;
			M.HEIGHT = window.innerHeight + 500;
			M.init();
		};
	},
	...
	draw: function() {
		...
		for (var i = 0; i < M.COLUMNS; i++) {
			...
			if (M.codes[i][0].canvas) {
				...
				if ((M.codes[i][0].position.y - height) < M.HEIGHT + 500){
					M.codes[i][0].position.y += velocity;
				} else {
	...
	createLines: function() {
		...
		while (linesYBlack < M.HEIGHT + 500) {
		...
		while (linesYWhite < M.HEIGHT + 500) {
